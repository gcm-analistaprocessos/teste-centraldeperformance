...
// usar este endpoint pra tentar logins contra a Senior).
...
export default async (req) => {
  ...
  try {
    // Timeout explícito: sem isso, se a rede até a Senior travar (porta
    // bloqueada, sem resposta), esta função ficava pendurada indefinidamente
    // e o navegador nunca via erro nem sucesso — só "carregando" pra sempre.
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 15000);
    let upstream;
    try {
      upstream = await fetch(SOAP_ENDPOINT_URL, {
        method: "POST",
        headers: {
    ...
    console.log("Senior SOAP respondeu, HTTP", upstream.status, "- primeiros 300 chars:", xml.slice(0, 300));
    ...
    // CONFIRMADO por teste real (login válido e inválido de verdade):
    // gcmAutenticado NÃO é booleano — vem "0" no login válido e "-1" no
    // inválido, então não serve de sinal. O sinal correto é o "status"
    // dentro de <retorno>, em formato de código HTTP: 200 = válido, 400 = inválido.
    const autenticado = statusRetorno === "200";

    console.log("Diagnóstico de autenticação -> status:", JSON.stringify(statusRetorno),
      "| erroExecucao:", JSON.stringify(erroExecucao),
    ...
    if (err?.name === "AbortError") {
      console.error("Senior SOAP: timeout de 15s sem resposta.");
      return json({ autenticado: false, mensagem: "O serviço da Senior não respondeu a tempo (rede/firewall). Tente novamente." }, 504);
    }
